...
const InputUploadMutiImage = ({ fieldName, form, label, inputProps, isViewOnly }: Props) => {
    ...
    const [currentSelectFiles, setcurrentSelectFiles] = useState<string[]>();
    const [openModal, setOpenModal] = useState(false);
    const [currentTab, setCurrentTab] = useState("media");

    useEffect(() => {
        setcurrentSelectFiles(formValue);
    }, [formValue]);
    ...
    return (
        <Form.Item label={label}>
            <div>
                {form.getFieldValue(fieldName) && form.getFieldValue(fieldName).length > 0 ? (
                    <div className="flex gap-4 flex-wrap ">
                        {form.getFieldValue(fieldName).map((img: string) =>
                            <div className="relative w-max">
                                {!isViewOnly && <X size={15}
                                    className="absolute bg-white border top-2 right-2 text-black z-20 cursor-pointer"
                                    onClick={(e) => {
                                        e.stopPropagation()
                                        setcurrentSelectFiles(currents => currents?.filter(item => item !== img))
                                        if (formValue.length === 1) form.resetFields([fieldName])
                                        else {
                                            form.setFieldValue(fieldName, formValue.filter((item: string) => item !== img))
                                        }
                                    }}
                                />}
                                <img
                ...
                <Tabs activeKey={currentTab} onChange={(key) => setCurrentTab(key)}>
                    <TabPane tab="Media" key="media">
                            ...
                            <div className="grid grid-cols-3 gap-4 max-h-[500px] overflow-y-auto">
                                {uploaded?.data.map((item: any) => (
                                    <div
                                        ...
                                        onClick={() => {
                                            if (currentSelectFiles?.includes(item.url)) {
                                                setcurrentSelectFiles(currents => currents?.filter(file => file !== item.url))
                                            } else {
                                                setcurrentSelectFiles(currents => currents ? [...currents, item.url] : [item.url])
                                            }
                                        }}
                                    >
                                        {currentSelectFiles?.includes(item.url) && (
                                            <CheckCircleTwoTone
                                                twoToneColor="#52c41a"
                        ...
                        <div className="mt-4 w-full justify-between flex gap-2">
                            <Button
                                onClick={() => {
                                    form.setFieldValue(fieldName, currentSelectFiles);
                                    setOpenModal(false);
                                }}
                            >
                                Select
...
export default InputUploadMutiImage;
